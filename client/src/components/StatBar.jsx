...
export function StatBar({ label, value, max = 20 }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '2px' }}>
      <span style={{ fontFamily: FONT, fontSize: FS.micro, color: T.textMuted, width: '60px', flexShrink: 0, textTransform: 'uppercase' }}>{label}</span>
      ...
      <span style={{ fontFamily: FONT, fontSize: FS.micro, color: T.textSecondary, width: '18px', textAlign: 'right' }}>{value}</span>
    </div>
  );
}
